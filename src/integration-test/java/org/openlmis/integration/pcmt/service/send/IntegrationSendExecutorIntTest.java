...
@SpringBootTest
@ActiveProfiles("test")
public class IntegrationSendExecutorIntTest {

  private static final ZonedDateTime baseTime = ZonedDateTime.now();

  @Autowired
  private IntegrationSendExecutor executor;

  @Test
  public void shouldGetTasksInQueueWithoutRemovingThem() {
    // given
    SendTestTask task1 = new SendTestTask(true, baseTime.plusNanos(1L));
    SendTestTask task2 = new SendTestTask(true, baseTime.plusNanos(2L));
    SendTestTask task3 = new SendTestTask(true, baseTime.plusNanos(3L));
